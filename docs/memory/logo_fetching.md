...
BATCH 2026-09-18 (AI-bio, +25, manifest 352->377, 23 SVG + 2 PNG). Two URL-parsing bugs cost the most time, both the same shape: **the file extension is NOT at the end of the URL**. (a) Commons imageinfo URLs carry `?...utm_campaign=imageinfo...`, so `url.rsplit('.',1)[-1]` wrote 12 files named `<Node>.org&utm_campaign=...`; sniff the magic bytes instead (`<svg`/`<?xml`, `PNG`, `ÿØÿ`). (b) companieslogo srcs carry `?t=<epoch>` and sometimes `&download=true`, so a regex ending in `\.(png|svg)"` matches nothing — strip the query first. companieslogo remains the best fallback: its sitemap.xml (12,527 slugs) plus the ticker embedded in the image filename gave 12/13 in one pass, and it serves **SVG** (`<TICKER>_BIG-<hash>.svg`), not just the >=1080 PNG the old notes assumed — prefer `_BIG` svg, skip the `.D-` white-on-dark variants unless the logo is genuinely white. Slug token-matching still mis-ranks (`Novo Nordisk`->novocure, `Relay Therapeutics`->c4-therapeutics, `Schrödinger`->hutter-schrantz-stahlbau-ag): keep a small HINTS map of exact slugs and verify each hint is really in the sitemap. Wikidata P154 covered the big pharma names (12/25) but had NO logo for the US small/mid-cap biotechs (Jazz, 10x, Nautilus, Recursion, Schrödinger, Absci, Relay, AbCellera, Certara, Simulations Plus, Ginkgo, Novo). Official-site scraping trap hit again: evaxion.ai serves five animation frames named `1of5logo.svg`...`5of5logo.svg` — a 374B fragment that passes the drawable-element check; take the site's `logo/evaxion.png` instead. Manifest: APPEND only. Sorting the keys (even preserving inner order) turned a +100-line diff into 812 insertions / 712 deletions. **SOURCE-OF-TRUTH TRAP (cost a whole commit on 2026-09-18):** `static/logos/` is the SOURCE; `web/public/logos/` is a COPY that `web/scripts/sync-data.mjs` rewrites on every `npm run sync` (which `graph_build.py --sync` runs). Editing `web/public/logos/manifest.json` looks right — git even shows the diff — and then the next build silently restores the 352-entry copy from static/, so the commit ships the logo FILES but not their manifest entries (sync copies static->public but never deletes extra files in public, which is why the images survived and the manifest did not). Always write new logos AND the manifest entry into `static/logos/`, then run `npm run sync`, then verify BOTH manifests have the key and that `mf[key]['file']` exists on disk in both dirs.
